    ...
    public Response postJson(String content) throws JSONException {
        ...
        try {
            if (json.has("eventType")) {
                ...
                Reward reward = ActionDAL.getRewardFromEventTypeAndObjectType(event.getEventType().getId(), event.getObject().getObjectType().getId());
                if(reward != null){
//                    PlayerRewardDAL.addRewardToPlayer(reward.getId(), event.getPlayer().getId());
                    json.put("earnedReward", reward.toJSON());
                }
            } else {
